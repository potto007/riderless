...
reading a trained decision head, were read against unridden in September 2026.
...
ideas are worth taking, and what unridden already does that none of them do.
...
Structurally the opposite of unridden: one prompt and one canvas per request,
...
Ranked by value to unridden against effort and risk. Every item below is either
pure post-processing over data unridden already records, harness work, or an
...
Unridden's first documented limitation is that confidence is uncalibrated and
...
of every frozen run are already on disk, because `unridden/api/mapping.py:110`
...
unridden already binds model and runtime hashes (`unridden/api/mapping.py:46-50`).

Two unridden-specific hazards the candidates do not state. A positive
temperature cannot move a Choice argmax, but Score is
`sum(i * p_i)` (`unridden/api/mapping.py:99`) and Noul is `P(true)`
(`unridden/api/mapping.py:105`), so a temperature does move two of the three
...
Unridden already computes the inputs and stops one step short: the scorer
builds a ten-bin reliability table at
`scripts/unridden/run_usecase_suites.py:440`, a majority baseline at `:462`, and
...
because unridden's confidences are saturated near 1.0 and a naive index cut
...
This is the measurement unridden's own design makes most load-bearing and has
never taken. Options are assigned to letters in caller order
(`unridden/api/compiler.py:107,114-119`), so permuting the options re-binds
every option to a different letter, and both a positional prior and a letter
prior land straight on the answer. Unridden proves sibling isolation at exactly
...
`scripts/unridden/compare_observations.py` already diffs two runs.
...
Unridden returns `confidence = max(probabilities)`
(`unridden/api/mapping.py:88`), which saturates and is not comparable across
...
unridden already computes, so they cost nothing at inference and can be added
as named additive fields or diagnostics without redefining `confidence`, which
is pinned as `max_probability_v1` at `unridden/api/schema.py:142`.
...
Unridden publishes paired comparisons whose significance is unstated: 0.928
...
This is the control missing from unridden's most hedged number. `README.md:229-232`
...
configuration. Today that movement is one undifferentiated figure. The unridden
...
that is the audit that would replace unridden's extrapolated no-reuse figure
...
Unridden's batched mode deliberately gives up sibling independence
...
that one question alone on the sequential path, which is the shape unridden
...
Unridden's `validate_alphabet` (`unridden/api/native/worker.cpp:303-334`) runs
...
prompt (`unridden/api/native/worker.cpp:292-299`).
...
- **Nonce-fenced or tag-fenced state block.** `jevmlx/jevmlx/engine.py:2387-2408`
  ...
  instructions (`prompts.py:14-22`). Unridden builds its prompt from bare
  markers (`unridden/api/compiler.py:108-120`), and its control-token rejection
  (`unridden/api/native/worker.cpp:170-177`) by design permits ordinary markup,
...
- **An "unknowable" partition scored on confidence only.**
  ...
  rather than by convention. Unridden's hierarchical suite already pairs
...
- **Blind confident-failure review with confidence-matched controls.**
  ...
  correction (`:89`). Unridden's suites are hand-authored, so some share of its
...
- **Committed golden prompt vectors, and a CI switch that fails on skipped
  ...
  (`open-alternative-jev/tests/conftest.py:10-18`). Unridden hashes the prompt
...
- **Grouped batching instead of an all-or-nothing fallback.**
  ...
  the fewest contiguous groups that fit. Unridden answers the entire request
  sequentially when the batched cell count exceeds the batched context
  (`unridden/api/native/worker.cpp:719-729`). Grouping would recover most of the
...
- **A one-option question answered without reading the model.**
  ...
  Unridden rejects it at the wire (`unridden/api/schema.py:60,73`). Doing this
...
- **Self-reported device memory as a delta from the pre-load baseline.**
  ...
  in the report that other processes are counted too. Unridden currently
...
- **Escaping caller control tokens instead of rejecting them**
  (`kev/kev/model.py:20-26` rewrites `<|name|>` to a lookalike). Unridden
...
- **Server-side derived facts injected into the state** (`kev/kev/api.py:66-91`
  appends computed date relationships). Unridden compiles exactly the state the
...
- **Cross-question conditioning.** openjev writes earlier chunks' answers into
  ...
  unridden's documented trap that a question cannot see its siblings
  (`README.md:242-244`), and both destroy the guarantee unridden can prove and
...
- **Cross-request caches.** jevmlx caches a neutral-context prior in a
  ...
  Unridden's isolation rule 5 (`docs/architecture.md:111-113`) says nothing
...
- **Top-k logprob readouts with an invented floor for missing labels**
  ...
  caller). Unridden owns the child and reads exact logits at validated label
...
- **Replacing the label alphabet with a trained head or a trie.**
  ...
  real cost. Both also replace the single clearest invariant unridden has, that
  an answer is one read of the final-position logits at validated single-token
  ids, and the trained-head version introduces weights unridden does not own
  and cannot hash the way it hashes the GGUF.
- **Heuristic script or language gating before inference**
  (`laya/laya/lang.py`). Unridden serves one model and has nothing to route
...
- **Provenance that is enforced, not merely recorded.** The install is hash
  ...
  (`unridden/api/mapping.py:46-84,131-132`). Among the eight, rizzo-flow hashes
...
- **The non-generation contract checked at the protocol boundary.**
  `unridden/api/mapping.py:51-54` rejects any result with a non-zero generated
...
- **Exact, unrounded probabilities from a full-vocabulary read.** kev rounds to
  ...
  candidates fabricate a floor for labels outside a top-k. Unridden returns the
...
- **A service failure model.** A documented error table with retryable flags,
  ...
  (`README.md:194-214`, `unridden/api/native_backend.py:352-379`).
- **Caller text cannot forge a turn boundary.** A 422 rather than a silent
  mutation (`unridden/api/native/worker.cpp:170-177`). Two candidates silently
  rewrite the caller's text instead, and one of those rewrites a token that is
  structurally load-bearing for its own readout.
- **A budget error instead of silent truncation.** Unridden returns 422
...
run needs a remote backend in `scripts/unridden/run_usecase_suites.py`, which
