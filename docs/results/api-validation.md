...
([`systemone/examples/api-v1-cases.json`](../../systemone/examples/api-v1-cases.json),
...
is [`scripts/systemone/validate_api_v1.py`](../../scripts/systemone/validate_api_v1.py);
failure paths are driven by
[`scripts/systemone/validate_api_failures.py`](../../scripts/systemone/validate_api_failures.py).
...
Run through `python -m systemone.api.cli run --gpu --diagnostics`, sequentially,
...
--explicit-package-bases systemone scripts`, and `pytest`, which is 53 tests
across `systemone/tests`. The larger validation run above was recorded against
the originating tree, which carried additional legacy modules and their tests;
only the API, its tests, and the harness were ported here.
