...
against riderless's own seven use-case suites on 2026-09-21. Two ran as
shipped. The third, `so1`, could not load riderless's model through its own
backends, so it was given a llama.cpp backend and run on the exact weights,
engine and kernels riderless uses, which isolates its prompt and readout design
...
| so1 separate, so1 packed | `ikermoel/open-alternative-jev` prompt builder and decider, with a llama.cpp backend written for this page (`riderless/api/native/so1_probe.cpp`) | the same GGUF as the riderless row, same sha256, same llama.cpp `v0.4.1` runtime | RTX 5090, one owned helper process, in-process calls |
...
| Suite | Questions | Trivial baseline | riderless | Kev-9B | Laya | so1 separate | so1 packed |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
riderless is ahead on all seven suites against Kev and Laya. Kev is behind on
...
| Cut | riderless | Kev-9B | Laya | so1 separate | so1 packed |
| --- | ---: | ---: | ---: | ---: | ---: |
...
| Only riderless correct | 76 | 438 | 30 | 97 |
...
candidate span and 6 in the guardrail suite; riderless's 30 over it are 15 in
the guardrail suite and 6 in hierarchical classification.

## Calibration, latency, memory

| Measure | riderless | Kev-9B | Laya | so1 separate | so1 packed |
| --- | ---: | ---: | ---: | ---: | ---: |
...
riderless (in-process wall clock around one call per case to an owned llama.cpp
helper), but not the same work: riderless answers a case one question at a
...
peak against a 5,382 MiB baseline for riderless (a 26B MoE at Q4), 24,416 MiB
...
helper, which holds the same model as riderless with a 4,096-token context
...
Kev is the closest independent model here. It trails riderless on every suite and
...
## so1 on riderless's own engine: the readout experiment

`ikermoel/open-alternative-jev` (import name `so1`) was chosen because it is
the one candidate that can be pointed at riderless's exact model, which turns
...
llama.cpp helper (`riderless/api/native/so1_probe.cpp`) that loads the same
...
to the same token ids the reference run recorded, and 200 of riderless's own
...
rendering. And riderless's content is carried over verbatim (serialized state,
instruction text, per-option rubric text, option order) but framed by so1's
prompt: so1's own instruction lines replace riderless's system instruction,
...
0.931 overall, 30 questions only riderless gets right against 17 only so1 gets
...
2.2 points (15 of riderless's 30 wins), and its confidence ranks correctness
...
latency than riderless, because it re-prefills the state for every question
where riderless reuses it.
...
written for it, it is larger and does cost accuracy. riderless's opt-in batched
...
- **so1 ran on a backend and a chat format it does not ship.** The llama.cpp
  ...
  built are the prompts the model saw; the prompt content is riderless's,
  folded into so1's frame, so the so1 rows measure so1's design on riderless's
...
| so1 driver and llama.cpp backend | `scripts/riderless/bench_competitor_so1.py` |
| so1 logits helper | `riderless/api/native/so1_probe.cpp` |
