...
from unridden.api.backend import BackendRequestError
...
async def test_routes_return_typed_results_profile_and_readiness() -> None:
    ...
    async with client_for(backend) as client:
        ...
        response = await client.post("/v1/decisions?diagnostics=true", json=REQUEST)
    ...
    assert profile["id"] == "local-gemma-unridden-v1"
...
@pytest.mark.asyncio
async def test_compatibility_alias_answers_exactly_like_the_primary_route() -> None:
    backend = DeterministicBackend()
    async with client_for(backend) as client:
        primary = await client.post("/v1/decisions?diagnostics=true", json=REQUEST)
        alias = await client.post("/v1/systemone?diagnostics=true", json=REQUEST)
...
@pytest.mark.asyncio
async def test_compatibility_alias_enforces_the_same_request_bounds() -> None:
    backend = DeterministicBackend()
...
async def test_request_and_transport_errors_are_structured() -> None:
    ...
    async with client_for(backend, max_request_bytes=200) as client:
        ...
        media = await client.post(
            "/v1/decisions", content=b"x", headers={"content-type": "text/plain"}
        )
...
async def test_concurrency_limit_rejects_busy_without_partial_work() -> None:
    ...
    backend.block = True
    async with client_for(backend) as client:
        first = asyncio.create_task(client.post("/v1/decisions", json=REQUEST))
        await backend.entered.wait()
        second = await client.post("/v1/decisions", json=REQUEST)
        backend.release.set()
...
async def test_internal_failures_are_structured_and_corruption_closes_backend(
    ...
    async with client_for(backend) as client:
        response = await client.post("/v1/decisions", json=REQUEST)
        health = await client.get("/health")
