// A logits server for competitor benchmarks, on unridden's own runtime.
//
...
// can load the GGUF unridden is measured on. This helper closes that gap: it
// loads the same file with the same llama_model_params and the same
// llama_context_params worker.cpp uses, so a so1 run and a unridden run
...
// Each readout object carries the same two facts unridden's own diagnostics
// carry at its readout position, `allowed_label_mass` and
// `full_vocabulary_argmax`, so a competitor row can be read beside a unridden
...
struct settings {
    ...
    // because a packed competitor prompt is longer than a unridden one.
    int context = 2048;
    int batch = 256;
    int ubatch = 256;
    int threads = 8;
...
// row comparable to a unridden row.
...
json score_sequence(
    ...
    for (const size_t count : unridden::prefill_chunks(
        ...
        for (const int32_t local : wanted) {
            ...
            const auto summary =
                unridden::summarize_vocabulary(vocabulary_logits, allowed);
            readouts.push_back({
...
int main(int argc, char ** argv) {
    try {
        ...
        // (unridden/api/native/worker.cpp), so the kernels selected here are
