...
`unridden-api worker fetch` downloads the release asset built for the
installed unridden version, checks it against the release's `SHA256SUMS`,
...
LOGGER = logging.getLogger("unridden.api.native.fetch")
...
def release_tag(requested: str | None) -> str:
    if requested is None:
        try:
            requested = installed_version("unridden")
        except PackageNotFoundError as error:  # pragma: no cover - install shape
            raise FetchError(
                "unridden is not installed, so its version is unknown; pass --version"
            ) from error
    return requested if requested.startswith("v") else f"v{requested}"


def download(url: str, destination: Path) -> Path:
    request = urllib.request.Request(url, headers={"User-Agent": "unridden-fetch"})
    try:
        with (
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="unridden-api worker fetch")
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        "Default: the installed unridden version",
    )
    parser.add_argument(
        "--repository",
