...
constexpr const char * PROMPT_VERSION = "unridden-gemma-choice-v1";
...
prepared_question prepare_question(
    ...
    for (size_t index = 0; index < prepared.labels.size(); ++index) {
        ...
        const int token_id = unridden::single_suffix_token(prepared.tokens, extended);
...
json evaluate_question(
    ...
    (void) unridden::stable_softmax(label_logits);
    const auto summary = unridden::summarize_vocabulary(
...
json evaluate_batched(
    ...
    if (prefix > 0) {
        ...
        size_t offset = 0;
        for (const size_t chunk : unridden::prefill_chunks(
