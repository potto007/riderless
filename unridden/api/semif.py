...
def import_semif_row(value: object) -> DecisionRequest:
    try:
...
def export_semif_row(request: DecisionRequest) -> SemIfRow:
...
def main() -> None:
    ...
    try:
        ...
        if args.direction == "to-api":
            ...
            converted = [
                export_semif_row(DecisionRequest.model_validate(row)) for row in raw
            ]
