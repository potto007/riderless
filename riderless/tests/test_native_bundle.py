...
from riderless.api.app import ApiConfig
...
@pytest.mark.parametrize(
    ("value", "message"),
    [
        ("/somewhere/riderless-worker", "relative"),
        ("../outside/riderless-worker", "escapes"),
        ("", "does not name"),
    ],
)
...
def test_packing_refuses_anything_that_is_not_the_bundle_layout(
    ...
    directory = tmp_path / "api-worker"
    directory.mkdir()
    _bundle(directory, executable="somewhere-else/riderless-worker")
...
def test_asset_names_carry_the_version_platform_and_flavor() -> None:
    assert (
        asset_name("0.2.0", "cuda13")
        == "riderless-worker-0.2.0-linux-x64-cuda13.tar.gz"
    )
