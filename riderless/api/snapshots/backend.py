...
Mirrors `riderless/api/native_backend.py`: one resident child, a verified
...
LOGGER = logging.getLogger("riderless.api.snapshots.native")
WORKER_PROTOCOL_BYTES = 4 * 1024 * 1024
WORKER_BINARY_NAME = "riderless-snapshot-worker"
...
class SnapshotNativeBackend:
    ...
    def _verify_files(self) -> VerifiedInstall:
        ...
        if manifest.get("protocol") != "riderless-snapshot-v1":
