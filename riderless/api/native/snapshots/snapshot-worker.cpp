// Riderless state-snapshot worker, profile split18-30-v1.
//
...
constexpr const char * PROTOCOL = "riderless-snapshot-v1";
constexpr const char * PROFILE = "split18-30-v1";
constexpr const char * MODEL_ID = "local-gemma-riderless-v1";
constexpr const char * CONTEXT_PROMPT_VERSION = "riderless-gemma-context-v1";
...
public:
    ...
    matrix run_upper(const matrix & h18_rows, llama_pos first_pos,
        ...
        size_t offset = 0;
        for (const size_t count : riderless::prefill_chunks(rows, static_cast<size_t>(config_.batch))) {
    ...
    void run_reference(const std::vector<llama_token> & tokens, matrix & h18, matrix & h30,
        ...
        h30.clear();
        size_t offset = 0;
        for (const size_t count : riderless::prefill_chunks(tokens.size(), static_cast<size_t>(config_.batch))) {
...
private:
    ...
    matrix run_tokens(llama_context * context, const std::vector<llama_token> & tokens,
        ...
        size_t offset = begin;
        for (const size_t count : riderless::prefill_chunks(end - begin, static_cast<size_t>(config_.batch))) {
            auto batch = llama_batch_get_one(const_cast<llama_token *>(tokens.data()) + offset,
                                             static_cast<int32_t>(count));
...
readout make_readout(const std::vector<double> & vocabulary, const std::vector<int> & label_ids, int top_k) {
    ...
    if (!label_ids.empty()) {
        ...
        (void) riderless::stable_softmax(result.label_logits);
        const auto summary = riderless::summarize_vocabulary(vocabulary, label_ids);
...
private:
    ...
    std::vector<int> label_ids_for(const json & labels_value, const rendered & full) {
        ...
        for (size_t index = 0; index < labels.size(); ++index) {
            ...
            const int token_id = riderless::single_suffix_token(full.tokens, extended);
