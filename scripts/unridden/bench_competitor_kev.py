"""Score Kev (github.com/jaredpalmer/kev) on the unridden use case suites.

Kev serves the same TypeSafe System One request shape unridden serves, so the
...
validated and hashed before the first request, exactly as the unridden driver
...
* Kev's Choice confidence is (p_max - 1/K) / (1 - 1/K) and its Score confidence
  ...
  unridden's max label probability. To keep the scorer like for like, the
  answers handed to score_answer carry unridden's own max-probability
...
Usage:
    python scripts/unridden/bench_competitor_kev.py \
...
LOGIT_NOTE = (
    ...
    "not unridden raw label logits and must not be compared as such."
)
CONFIDENCE_NOTE = (
...
def map_answers(case: Case, body: Row) -> tuple[dict[str, Answer], Row]:
    """Kev's answers as unridden Answer objects, plus per-question diagnostics."""
...
def confidence_block(
    ...
    unridden_style = [outcome.confidence for outcome in outcomes]
    paired = [
        (value, flag)
    ...
    return {
        "n": len(outcomes),
        "auroc_max_probability": auroc(unridden_style, correct),
        ...
        "note": (
            "auroc_max_probability uses unridden's confidence formula recomputed "
    ...
    write_json(
        args.out / "kev-extras.json",
        {
            ...
            "latency_note": (
                ...
                "includes JSON, HTTP and FastAPI overhead. The unridden "
