...
round trip is reported separately. Needs the GPU and the user's confirmation.
    ...
    python scripts/unridden/bench_snapshots.py --bundle build/snapshot-worker \\
...
from scripts.unridden.qualify_snapshots import (
    ANSWER_PREFIX,
    PROMPT_VERSION,
    Worker,
    context_messages,
...
def run(args: argparse.Namespace) -> Row:
    ...
    results: list[Row] = []
    scratch = Path(tempfile.mkdtemp(prefix="unridden-bench-"))
    try:
