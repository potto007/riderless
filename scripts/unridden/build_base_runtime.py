...
The worker in `unridden/api/native` is deliberately not built against whatever
...
The output directory is the `--base` argument of `unridden.api.native.build`
...
Example:
    ...
    python scripts/unridden/build_base_runtime.py --out build/llama-base \\
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "unridden/api/native/patches/gemma4-layer-range.patch",
    )
