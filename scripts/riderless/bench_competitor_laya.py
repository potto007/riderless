"""Score Laya (github.com/NandhaKishorM/laya) on the riderless use case suites.
...
same `{"model", "answers", "usage"}` payload riderless returns, with the same
...
riderless driver does, so the labels are frozen against the same bytes.

What this driver records, and does not paper over:

* Laya's context window is the checkpoint's `max_len` (1024 for
  `typed-decisions`) against riderless's 2048, and its option head is capped at
...
* Laya's confidence is normalised entropy, `1 - H(p)/log(k)`
  (laya/common.py:210-216), not riderless's max label probability. The answers
  handed to `score_answer` carry riderless's own max-probability confidence so
...
Usage:
    python scripts/riderless/bench_competitor_laya.py \
...
LOGIT_NOTE = (
    ...
    "They are not riderless raw label logits and must not be compared as such."
)
CONFIDENCE_NOTE = (
...
def laya_question(question: Question) -> Row:
    """One riderless question in the shape `Agent.system_one` documents.
...
def map_answers(case: Case, body: Row) -> tuple[dict[str, Answer], Row]:
    """Laya's answers as riderless Answer objects, plus per-question diagnostics."""
    ...
    for qid, question in case.request.questions.items():
        ...
        if question.type == "noul":
            ...
            # riderless orders a noul's labels true, false (compiler.py:83), so
...
def run(args: argparse.Namespace) -> int:
    ...
    riderless_confidence = [item.confidence for item in outcomes]
    ...
    write_json(
        args.out / "laya-extras.json",
        {
            ...
            "calibration_max_probability": calibration_block(
                riderless_confidence, was_correct
            ),
            ...
            "latency_note": (
                ...
                "every question of the case in one forward pass. The riderless "
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="a riderless observations.jsonl to compute the same calibration block for",
    )
    parser.add_argument(
        "--provenance",
        type=Path,
