...
constexpr const char * MODEL_ID = "local-gemma-systemone-v1";
constexpr const char * PROMPT_VERSION = "systemone-gemma-choice-v1";
...
prepared_question prepare_question(
    ...
    for (size_t index = 0; index < prepared.labels.size(); ++index) {
        ...
        const int token_id = systemone::single_suffix_token(prepared.tokens, extended);
...
std::pair<std::vector<std::string>, std::vector<int>> validate_alphabet(
    ...
    for (const char candidate : std::string(LABEL_CANDIDATES)) {
        ...
        try {
            ...
            const int token_id = systemone::single_suffix_token(prompt_tokens, extended);
...
json evaluate_question(
    ...
    const auto prefill = [&](size_t begin, size_t end) {
        size_t offset = begin;
        for (const size_t count : systemone::prefill_chunks(
    ...
    (void) systemone::stable_softmax(label_logits);
    const auto summary = systemone::summarize_vocabulary(
...
int main(int argc, char ** argv) {
    try {
        ...
        ggml_backend_load_all_from_path(SYSTEMONE_BACKEND_DIR);
