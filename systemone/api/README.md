...
The API model id is `local-gemma-systemone-v1`. It names this local
...
python -m systemone.api.native.build \
  --base build/llama-base \
  --output build/api-worker
```

The result is `build/api-worker/build/systemone-api-worker`. Its sibling
...
app = create_app(
    ApiConfig(
        ...
        worker_path=Path("build/api-worker/build/systemone-api-worker"),
...
```bash
python -m systemone.api.cli serve \
...
The supported routes are `POST /v1/systemone`, `GET /v1/models`, and
...
`systemone.api.native` logger at DEBUG or inspect the bounded
...
```bash
python -m systemone.api.cli run \
...
```bash
python -m systemone.api.semif to-api \
  --input semif.jsonl --output requests.jsonl
python -m systemone.api.semif from-api \
