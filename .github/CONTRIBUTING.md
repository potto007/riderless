# Contributing to systemone-local

Bug reports, fixes, tests, documentation, and new use-case suites are welcome.
systemone-local is in early development and has no tagged release, so the HTTP
...
Search [existing issues](https://github.com/OWNER/systemone-local/issues)
...
Issues labeled [good first issue](https://github.com/OWNER/systemone-local/labels/good%20first%20issue)
or [help wanted](https://github.com/OWNER/systemone-local/labels/help%20wanted)
...
```sh
git clone https://github.com/YOUR_USERNAME/systemone-local.git
cd systemone-local
git remote add upstream https://github.com/OWNER/systemone-local.git
...
`systemone/tests` show the pattern.
...
uv run ruff check .
uv run mypy --strict --explicit-package-bases systemone scripts
uv run pytest
```

`uv run ruff format .` applies the formatting rather than just reporting it.

The C++ helper test for `systemone/api/native/worker-utils.h` is
self-contained and needs no llama.cpp:

```sh
mkdir -p build
c++ -std=c++17 -Wall -Wextra -Werror -O2 \
  -o build/worker-utils-test \
  systemone/api/native/worker-utils-test.cpp
./build/worker-utils-test
```
...
2. Push your branch to your fork and open a PR against `OWNER/systemone-local`'s
